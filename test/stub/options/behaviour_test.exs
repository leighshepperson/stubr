defmodule Stubr.Stub.BehaviourTest do
  ...
  describe "behaviour option" do
    ...
    test "returns a warning if the stub does not implement a behaviour" do
      assert capture_io(:stderr, fn ->
        SUT.create_stub!([{:bar, fn _ -> 4 end}], behaviour: FooBehaviour) end
      ) =~ "undefined behaviour function foo/1"
    end

    test "returns a warning if the stub does not implement the correct overload" do
      assert capture_io(:stderr, fn ->
        SUT.create_stub!([{:foo, fn(_, _) -> 4 end}], behaviour: FooBehaviour) end
      ) =~ "undefined behaviour function foo/1"
    end
    ...
    test "returns a warning if the behaviour is undefined" do
      ...
      assert capture_io(:stderr, fn ->
        SUT.create_stub!([{:foo, fn _ -> 4 end}], behaviour: NoBehaviour) end
      ) =~ "behaviour Stubr.Stub.BehaviourTest.NoBehaviour undefined"
    end

    test "succeeds if the stub implements a behaviour" do
      assert capture_io(:stderr, fn ->
        SUT.create_stub!([{:foo, fn _ -> 4 end}], behaviour: FooBehaviour) end
      ) == ""
    end
